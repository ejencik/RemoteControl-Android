...
//	  inflater = i;
      ...
      cm = new ConnectionCredentialsManager(getActivity());
			...
			AsyncTask< Integer, Void, String> getGraphDataTask = new AsyncTask< Integer, Void, String>() {
				...
				protected String doInBackground( Integer ... params) {
					...
					try {
						return scriptCaller.callCGIScriptAndGetGraphDataOutside(device);
	...
	//			Toast.makeText(getActivity(), "GraphCgiScriptCaller result: " + result, Toast.LENGTH_LONG).show();
					...
					openChart(v,graph_valuesUpDown[0],graph_valuesUpDown[2],true);
                    openChart(v,graph_valuesUpDown[1],graph_valuesUpDown[1],false);
                    return v;
	   			}
	...
	private void openChart(View v, String graph_values, String graph_values2, Boolean pos){
		...
		int length_data =data_y.length>data_y2.length?data_y.length:data_y2.length;
		...
		for(int i=0;i<length_data;i++){
			...
			if (i< data_y.length) {dataSeries.add(i,Double.parseDouble(data_y[i]));} else {dataSeries.add(i,0);};
			if (i< data_y2.length) {dataSeries2.add(i,Double.parseDouble(data_y2[i]));} else {dataSeries2.add(i,0);};
...
//		multiRenderer.setChartTitle(min_val+"/"+max_val);
		multiRenderer.setChartTitle(pos?"Temperature":"Daylight");

		multiRenderer.setXTitle("h");
		multiRenderer.setYTitle(pos?"C":"Lx");
		...
		for(int i=0; i< length_data;i++){		multiRenderer.addXTextLabel(i, data_x[i]);
		}
